...
def affine_backward(dout, cache):
    ...
    dw = np.dot(x.reshape(x.shape[0], np.prod(x.shape[1:])).T, dout).reshape(w.shape)  # FFFFUUUUUUUU
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # Forward pass
        # 1 mu (D,) - mean of a minibatch
        mu = 1 / float(N) * np.sum(x, axis=0)

        # 2 xmu (N,D) - x without shift
        xmu = x - mu

        # 3 carre (N,D)
        carre = xmu**2

        # 4 var (D,) - variance of minibatch
        ...
        # 7 va2 (N,D) - normalized x
        va2 = xmu * invvar

        # 8 va3 (N,D)
        va3 = gamma * va2

        # 9 out (N,D) - final scale&shift
        out = va3 + beta
